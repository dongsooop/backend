...
5. 문서와 코드에서 사용하는 용어는 `docs/glossary.md`를 기준으로 한다.
...
`docs/glossary.md`는 프로젝트 용어의 단일 원본이다.

- 같은 개념에 여러 이름을 사용하지 않는다.
- 새로운 용어가 필요하면 기존 용어와 중복되는지 먼저 확인한다.
- 기존 용어의 의미를 변경하면 영향을 받는 도메인 문서를 함께 검토한다.
