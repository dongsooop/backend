...
회원 탈퇴 시 기기는 익명 기기로 전환되므로(`MemberDevice.bindMember(null)`) 연동과 과제 데이터는 기기에 그대로 남는다. 기기가 삭제되면 FK `ON DELETE CASCADE`로 함께 지워진다.
...
6. 연동 해제는 `DELETE /eclass/link`. 회원 탈퇴 시 연동과 과제 데이터를 함께 삭제한다.
...
| `token_valid_until` | TIMESTAMP nullable | 이클래스가 알려주는 토큰 만료 시각(확인되면 저장, 5.5절). 만료 전 선제 재발급에 사용 |
...
모든 엔드포인트는 `authentication.path.all`에 `/eclass/**`로 등록해 비회원도 호출할 수 있다(기기 헤더로 식별). 토큰 검증 실패(`invalidtoken`)는 400 커스텀 예외로 앱에 "토큰이 유효하지 않음"을 명확히 준다.
...
- 주기: **하루 4회** (06:30, 12:30, 18:30, 23:30 KST, 설정값). 아침 8시 발송 전 06:30 수집이 반드시 돌아 최신 제출 상태로 발송된다.
- 대상: `eclass_link.status = ACTIVE` 전원. 회원별 처리를 스레드 풀(설정값, 기본 4)로 병렬화하되 회원 하나 안의 호출은 순차.
- 회원 1명 처리:
  1. `mod_assign_get_assignments` 1회 → 응답 전체에서 `duedate`가 수집 창 안인 과제만 추림.
  2. 추린 과제마다 `mod_assign_get_submission_status(assignid)` 호출 → `submitted` 갱신. 이미 `submitted=true`인 과제는 다시 묻지 않는다(제출 취소는 드물고, 놓쳐도 알림이 안 갈 뿐 잘못 가진 않음).
  3. upsert. 이번 응답에 없는데 DB에 있는 창 안 과제는 `removed_at` 기록.
  4. `last_synced_at` 갱신.
- 호출량: 회원당 1 + (창 안 미제출 과제 수). 학기 중 평균 5건 잡으면 회원 1,000명 연동 시 1회 수집에 약 6,000 호출. 요청 간 딜레이(기본 100ms)와 스레드 4개면 약 3분. 부하가 문제면 주기·스레드·딜레이를 yml에서 조정.
- 실패 처리:
  ...
  - 이클래스 전체 장애로 연속 실패 비율이 임계치(설정, 기본 50%)를 넘으면 해당 주기 중단.
...
- 발송 경로: `CalendarNotificationImpl.saveAndSendForMember`와 같은 흐름 — 알림함 저장 후 `MemberDeviceService.getDeviceByMember(조건: 회원 목록, ECLASS_ASSIGNMENT)`로 켜둔 기기만 발송. 비회원 발송은 없다(연동 자체가 회원 전용).
...
- **토큰 수명**: 영구라고 가정하지 않는다. Moodle 토큰은 (1) 관리자가 정한 유효기간이 지나면, (2) 사용자가 비밀번호를 바꾸면, (3) 사용자가 이클래스 "보안키 관리"에서 삭제하면 무효가 된다. 이클래스의 실제 유효기간은 로그인 후 `/user/managetoken.php`(환경설정 → 보안키)의 "유효기간" 열에서 확인한다 — **배포 전 확인 항목**. 유효기간이 있으면 연동 시 그 값을 `token_valid_until`에 저장하고 만료 3일 전에 "이클래스 연동이 곧 만료됩니다" 알림을 보내 미리 재연동하게 한다. 유효기간이 없더라도(값이 비어 있음) 비밀번호 변경으로 언제든 끊길 수 있으므로 만료 감지 → 재연동 안내 흐름(4절 7번)은 그대로 둔다.
  ...
  **선제 재발급**: `token_valid_until`이 있으면 만료 3일 전 수집 시점에 미리 `ECLASS_RELINK`를 보내, 만료로 수집이 끊기는 공백 자체를 없앤다. `login/token.php`는 아직 유효한 토큰이 있으면 같은 토큰을 돌려주므로, 앱은 재발급 응답의 토큰이 기존과 같으면 서버 호출을 생략하고 만료 후 다시 시도한다(앱 로직). 이클래스가 만료 없는 토큰을 주는 것으로 확인되면 이 항목은 자연히 동작하지 않는다.
...
- 회원 탈퇴(`DELETE /member`) 시 `eclass_link`, `eclass_assignment` 삭제.
...
| 교수가 마감 연장 | 다음 수집에서 `due_at` 갱신. 별도 알림 없음(후속 후보) |
...
- 토큰 만료·재발급: invalidtoken → EXPIRED + 사일런트 푸시 1회 + `relink_requested_at` 기록, 24시간 내 재연동 시 보이는 알림 없음, 24시간 경과 시 보이는 알림 1회(두 번째 주기엔 없음), 재연동 시 ACTIVE 복귀 + 즉시 수집, `token_valid_until` 3일 전 선제 사일런트 푸시 1회, 값이 없으면 선제 발송 없음
...
1. 이클래스 연동 화면: 아이디·비밀번호 입력 → `login/token.php` 직접 호출 → 토큰을 `POST /eclass/link`로 전달. 비밀번호는 앱 메모리에서만 쓰고 저장하지 않는다.
