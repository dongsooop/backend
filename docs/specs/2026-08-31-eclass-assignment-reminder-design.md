...
- **토큰 수명**: 영구라고 가정하지 않는다. Moodle 토큰은 (1) 관리자가 정한 유효기간이 지나면, (2) 사용자가 비밀번호를 바꾸면, (3) 사용자가 이클래스 "보안키 관리"에서 삭제하면 무효가 된다. 어느 경우든 **만료를 감지한 뒤 재발급하는 흐름(4절 7번) 하나로 처리한다.** 실제 유효기간은 서버가 알 방법이 없다. Moodle API에 만료 시각을 주는 함수가 없고, 웹의 보안키 관리 페이지(`/user/managetoken.php`)는 **학교가 막아두어 로그인 상태에서도 404**다(2026-09-03 확인. 환경설정 페이지에도 관련 링크가 없다).

관측된 사실은 하나다 — 2026-08-31에 발급한 토큰이 2026-09-03에 같은 값으로 반환됐다(`login/token.php`는 유효한 토큰이 있으면 재사용한다). 즉 **최소 3일 이상 유효**하며 세션성 토큰이 아니다. 상한은 미확정이다.

그래서 유효기간을 아는 것에 설계를 걸지 않는다. 앱이 만료를 감지해 자동 재발급하면 유효기간이 얼마든 사용자 경험이 같아지므로, 이 값은 "자동 재발급을 앱 1차에 넣을지"를 판단할 때만 쓰인다. 정확한 값이 필요하면 학교 IT에 문의한다(IP 허용 요청과 함께).
