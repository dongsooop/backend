...
public class LoginNotificationServiceImpl implements LoginNotificationService {
     ...
     *
     * <p>클라이언트가 제어하는 값({@code ipAddress}, {@code userAgent})은
     * HTML 인젝션을 방지하기 위해 {@link HtmlUtils#htmlEscape}로 이스케이프한다.
     * 로그인 시각은 KST(Asia/Seoul) 기준으로 표시한다.
     *
    ...
    private String injectLoginContext(String html, String ipAddress, String userAgent) {
        String loginTime = LocalDateTime.now(KST).format(LOGIN_TIME_FORMATTER);
        String ip = HtmlUtils.htmlEscape(
                (ipAddress != null && !ipAddress.isBlank()) ? ipAddress : "알 수 없음");
        String device = HtmlUtils.htmlEscape(
                (userAgent != null && !userAgent.isBlank()) ? userAgent : "알 수 없음");
        return html
     ...
     * 저장·발송 중 예외가 발생해도 로그인 흐름에 전파하지 않고 에러 로그만 기록한다.
     *
    ...
    private void sendPushNotification(Long memberId) {
        try {
            ...
            log.error("Failed to send push notification for member {}", memberId, e);
        }
    }
}
